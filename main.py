...
if __name__ == "__main__":
    ...
    pic_sub_path.append(Constant.RESULT_FOLDER_NAME + '/' + Constant.SBP_FOLDER_NAME + '/' + 'LF/')
    ...
    for i in range(0, pic_sub_path.__len__()):
        ...
        only_train_csv_files = [f for f in listdir(root_path + type_train_sub_path) if isfile(join(root_path +
                                type_train_sub_path, f)) & f.endswith('.csv') & (Constant.FILE_NAME_FILTER in f)]  # & f.startswith('a')
        only_test_csv_files = [f for f in listdir(root_path + type_test_sub_path) if isfile(join(root_path +
                                type_test_sub_path, f)) & f.endswith('.csv') & (Constant.FILE_NAME_FILTER in f)]  # & f.startswith('a')
